...
export default function App() {
  ...
  const [bookings, setBookings] = useState(() => {
    try { return JSON.parse(localStorage.getItem("sg_bookings") || "[]"); } catch { return []; }
  });
                ...
                {bookings.map((b,i) => (
                  <div key={b.id} className="card" style={{ borderRadius:20, overflow:"hidden", boxShadow:"0 4px 18px rgba(45,138,94,0.1)", animationDelay:`${i*0.06}s`, border:"2px solid #C8EDD8", background:"#fff" }}>
                    ...
                    <div style={{ padding:"12px 16px 14px" }}>
                      ...
                      <button onClick={() => cancelBooking(b.key, b.id)} style={{ width:"100%", padding:"10px", borderRadius:14, border:"2px dashed #C8EDD8", background:"#F5FFFA", color:"#999", fontWeight:900, fontSize:12, cursor:"pointer", fontFamily:"inherit" }}>取消預約 🍃</button>
                    </div>
                  </div>
                ))}
              </div>
