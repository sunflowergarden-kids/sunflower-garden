...
const categories = ["全部","手工藝術","感官遊戲","STEAM","語言文學","烹飪","音樂"];
...
export default function App() {
  ...
  const [form, setForm] = useState({ parent:"", child:"", phone:"", age:"", notes:"" });
  const [bookings, setBookings] = useState([]);
  const [user, setUser] = useState(null);
  const [authStep, setAuthStep] = useState("phone"); // phone | otp | done
  ...
  const loadFromSheets = (action) => new Promise((resolve, reject) => {
    const id = "frame_" + Math.random().toString(36).slice(2);
    const cb = "cb_" + id;
    let done = false;
    ...
    script.onload = () => { setTimeout(() => { if (!done) { done=true; reject(new Error("timeout")); delete window[cb]; script.remove(); } }, 8000); };
    script.onerror = () => { if (!done) { done=true; reject(new Error("script error")); delete window[cb]; script.remove(); } };
    document.head.appendChild(script);
  });

  // Load courses + dates from Sheets
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        ...
        const [cData, dData] = await Promise.all([
          loadFromSheets("getCourses"),
          loadFromSheets("getDates"),
        ]);
        const processed = (Array.isArray(cData) ? cData : []).map((c, i) => ({
          ...c,
          id: String(c.id),
          price: Number(c.price),
          totalSeats: Number(c.totalSeats),
          bg: c.bg || DEFAULT_BG[i % DEFAULT_BG.length],
        }));
        const processedDates = (Array.isArray(dData) ? dData : []).map(d => ({
          ...d,
  ...
  const loadBookings = async (uid) => {
    try {
      ...
      setBookings(data);
    } catch (e) { console.log("Load bookings error:", e); }
  };
  ...
  const verifyOTP = async () => {
    ...
    try {
      ...
      setAuthError("驗證碼錯誤，請重試");
    }
    setAuthLoading(false);
  };

  const getDatesForClass = (classId) => allDates.filter(d => String(d.courseId) === String(classId));
  ...
  const confirm = async () => {
    ...
    try {
      await fetch(SHEETS_URL, {
        method: "POST",
        redirect: "follow",
        body: JSON.stringify({ action:"addBooking", ...newBooking }),
      });
    } catch (e) {
  ...
  const confirmPayment = async () => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, "bookings"), {
        ...
        phone: user ? user.phoneNumber : phone,
  ...
  const cancelBooking = async (key, id) => {
    try {
      await fetch(SHEETS_URL, {
        method: "POST",
        redirect: "follow",
        body: JSON.stringify({ action:"cancelBooking", id }),
      });
    } catch (e) {}
  ...
  return (
    <div style={{ fontFamily:"'Nunito','PingFang HK','Noto Sans HK',sans-serif", background:"#F0FAF2", minHeight:"100vh", maxWidth:430, margin:"0 auto", position:"relative", overflowX:"hidden" }}>
      <div id="recaptcha-container" />

      {/* AUTH SCREEN */}
      {authStep !== "done" && (
        <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"24px", background:"linear-gradient(160deg,#F0FAF2,#E0F5E8)" }}>
          <div className="sun-pulse" style={{ width:90, height:90, borderRadius:"50%", overflow:"hidden", marginBottom:20, boxShadow:"0 6px 24px rgba(45,138,94,0.3)" }}>
          ...
          <div style={{ width:"100%", maxWidth:360, background:"#fff", borderRadius:24, padding:"28px 24px", boxShadow:"0 8px 32px rgba(45,138,94,0.15)" }}>
            {authStep === "phone" && (
              <>
                ...
                <div style={{ display:"flex", gap:8, marginTop:6, marginBottom:16 }}>
                  ...
                  <input type="tel" placeholder="9123 4567" value={phone} onChange={e => setPhone(e.target.value.replace(/\D/g,""))} maxLength={8}
                ...
                <button onClick={sendOTP} disabled={phone.length < 8 || authLoading} style={{ width:"100%", padding:"14px", borderRadius:16, border:"none", background: phone.length<8?"#C8EDD8":"linear-gradient(135deg,#52B788,#2D8A5E)", color:"#fff", fontWeight:900, fontSize:15, cursor:"pointer", fontFamily:"'Baloo 2',cursive", boxShadow: phone.length<8?"none":"0 4px 18px rgba(45,138,94,0.42)" }}>
            ...
            {authStep === "otp" && (
              <>
                ...
                <label style={{ fontSize:11, fontWeight:900, color:"#2D8A5E", textTransform:"uppercase", letterSpacing:0.8 }}>6位驗證碼</label>
                <input type="number" placeholder="123456" value={otp} onChange={e => setOtp(e.target.value)} maxLength={6}
                  style={{ width:"100%", padding:"14px", borderRadius:12, border: otp.length===6?"2.5px solid #52B788":"2.5px solid #C8EDD8", fontSize:20, fontWeight:800, color:"#1B4D32", background:"#fff", textAlign:"center", marginTop:6, marginBottom:16, boxSizing:"border-box", fontFamily:"inherit" }} />
                {authError && <div style={{ color:"#E8855B", fontSize:12, fontWeight:700, marginBottom:10 }}>{authError}</div>}
                <button onClick={verifyOTP} disabled={otp.length < 6 || authLoading} style={{ width:"100%", padding:"14px", borderRadius:16, border:"none", background: otp.length<6?"#C8EDD8":"linear-gradient(135deg,#52B788,#2D8A5E)", color:"#fff", fontWeight:900, fontSize:15, cursor:"pointer", fontFamily:"'Baloo 2',cursive", boxShadow: otp.length<6?"none":"0 4px 18px rgba(45,138,94,0.42)" }}>
      ...
      <div style={{ background:"linear-gradient(145deg,#52B788,#2D8A5E,#1B5E42)", padding:"24px 20px 28px", borderRadius:"0 0 32px 32px", boxShadow:"0 8px 32px rgba(29,110,70,0.38)", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", bottom:-25, left:-15, width:110, height:110, borderRadius:"50%", background:"rgba(255,255,255,0.07)" }} />
        <div style={{ position:"relative", display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div>
            ...
            <div style={{ fontFamily:"'Baloo 2',cursive", fontSize:25, fontWeight:800, color:"#fff", lineHeight:1.15, textShadow:"0 2px 8px rgba(0,0,0,0.15)" }}>Sunflower Garden</div>
      ...
      {/* TABS */}
      <div style={{ display:"flex", gap:10, padding:"12px 14px 0", position:"sticky", top:0, zIndex:10, background:"#F0FAF2" }}>
        {[{key:"browse",icon:"🔍",label:"瀏覽課程"},{key:"bookings",icon:"🌟",label:"我的預約"}].map(t => (
      ...
      {error && !loading && (
        <div style={{ margin:"20px 14px", background:"#FFF0F0", borderRadius:16, padding:"16px", border:"1.5px solid #FFB3B3", textAlign:"center" }}>
          ...
          <button onClick={() => window.location.reload()} style={{ marginTop:12, background:"linear-gradient(135deg,#52B788,#2D8A5E)", color:"#fff", border:"none", borderRadius:12, padding:"8px 20px", fontWeight:900, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>重新載入</button>
        </div>
      )}
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {filtered.length === 0 && !loading && (
              <div style={{ textAlign:"center", padding:"60px 20px" }}>
                ...
                <div style={{ fontFamily:"'Baloo 2',cursive", fontWeight:800, fontSize:16, color:"#A0C8B0" }}>未有相關課程</div>
              </div>
            )}
          </div>
        </div>
          ...
          <div style={{ borderRadius:22, overflow:"hidden", marginBottom:14, boxShadow:"0 6px 24px rgba(45,138,94,0.18)" }}>
            <div style={{ background:selectedClass.bg, padding:"16px 18px", position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                ...
                <div>
                  <div style={{ fontFamily:"'Baloo 2',cursive", fontSize:19, fontWeight:800, color:"#fff", textShadow:"0 1px 4px rgba(0,0,0,0.18)" }}>{selectedClass.name}</div>
                ...
                <div key={d.date} className="date-card" style={{ background:"#fff", borderRadius:18, padding:"12px 14px", boxShadow:"0 3px 14px rgba(0,0,0,0.07)", border:booked?"2.5px solid #52B788":full?"2.5px solid #eee":"2.5px solid #E8F5EE", animationDelay:`${i*0.07}s` }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <div style={{ width:48, height:48, borderRadius:14, background:booked?"linear-gradient(135deg,#B5E48C,#52B788)":full?"#f0f0f0":selectedClass.bg, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                        <div style={{ fontFamily:"'Baloo 2',cursive", fontSize:13, fontWeight:900, color:booked||!full?"#fff":"#ccc", textAlign:"center" }}>{d.date}</div>
                      </div>
                      <div>
                  ...
                  <div>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                      <span style={{ fontSize:10, fontWeight:800, color:"#ccc", letterSpacing:0.5 }}>座位情況</span>
        ...
        <div style={{ padding:"16px 14px 110px" }}>
          {bookings.length === 0 ? (
            <div style={{ textAlign:"center", padding:"56px 20px" }}>
              ...
              <button onClick={() => setActiveTab("browse")} className="bookbtn" style={{ background:"linear-gradient(135deg,#52B788,#2D8A5E)", color:"#fff", border:"none", borderRadius:20, padding:"13px 28px", fontWeight:900, fontSize:14, cursor:"pointer", fontFamily:"inherit", boxShadow:"0 4px 18px rgba(45,138,94,0.4)" }}>瀏覽課程 🔍</button>
            </div>
          ) : (
            <>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                {bookings.map((b,i) => (
                  <div key={b.id} className="card" style={{ borderRadius:20, overflow:"hidden", boxShadow:"0 4px 18px rgba(45,138,94,0.1)", animationDelay:`${i*0.06}s`, border:"2px solid #C8EDD8", background:"#fff" }}>
        ...
        <div className="modal-bg" onClick={() => setShowModal(false)} style={{ position:"fixed", inset:0, background:"rgba(10,50,30,0.5)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:200, backdropFilter:"blur(6px)" }}>
          <div className="modal-sheet" onClick={e => e.stopPropagation()} style={{ background:"#F0FAF2", borderRadius:"26px 26px 0 0", width:"100%", maxWidth:430, padding:"18px 20px 36px", maxHeight:"92vh", overflowY:"auto" }}>
            ...
            <div style={{ background:selectedClass.bg, borderRadius:18, padding:"12px 16px", marginBottom:16, display:"flex", alignItems:"center", gap:12, position:"relative", overflow:"hidden" }}>
              <div style={{ position:"absolute", top:-15, right:-15, width:70, height:70, borderRadius:"50%", background:"rgba(255,255,255,0.18)" }} />
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              {[{label:"👩 家長／監護人姓名",key:"parent",ph:"例如：陳美玲",req:true},{label:"🧒 小朋友姓名",key:"child",ph:"例如：陳小明",req:true},{label:"📱 聯絡電話",key:"phone",ph:"例如：9123 4567",req:true},{label:"🎂 小朋友年齡",key:"age",ph:"例如：4歲",req:false}].map(f => (
                <div key={f.key}>
                  ...
                  <input type={f.key==="phone"?"tel":"text"} placeholder={f.ph} value={form[f.key]} onChange={e => setForm(p => ({...p,[f.key]:e.target.value}))} style={{ width:"100%", padding:"12px 14px", borderRadius:14, border:form[f.key]?"2.5px solid #52B788":"2.5px solid #C8EDD8", fontSize:14, fontWeight:700, color:"#1B4D32", background:"#fff", boxSizing:"border-box" }} />
                </div>
              ))}
              <div>
